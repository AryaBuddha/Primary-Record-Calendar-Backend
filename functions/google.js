...
const { doesUserExist, refreshDatabase } = require("../database.js");
...
const saveGoogleRefresh = async (authCode) => {
  let { tokens } = await oauthClient.getToken(authCode);
...
const addGoogleEvent = async (eventDetails, userDataObj, callback) => {
  ...
  eventDetails.start.timeZone = "America/Indianapolis";
  eventDetails.end.timeZone = "America/Indianapolis";
  delete eventDetails.reminders.overrides;
  eventDetails.reminders.useDefault = true;
